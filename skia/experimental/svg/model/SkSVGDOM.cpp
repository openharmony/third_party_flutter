...
#include "experimental/svg/model/SkSVGXMLDOM.h"
...
void parse_node_attributes(const SkSVGXMLDOM& xmlDom, const SkDOM::Node* xmlNode,
...
sk_sp<SkSVGNode> construct_svg_node(const SkSVGXMLDOM& dom, const ConstructionContext& ctx,
...
sk_sp<SkSVGDOM> SkSVGDOM::MakeFromDOM(const SkSVGXMLDOM& xmlDom) {
...
sk_sp<SkSVGDOM> SkSVGDOM::MakeFromStream(SkStream& svgStream, uint64_t svgColor) {
    SkSVGXMLDOM xmlDom;
    if (!xmlDom.build(svgStream, svgColor)) {
        return nullptr;
    }
