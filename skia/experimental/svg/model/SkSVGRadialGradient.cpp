...
sk_sp<SkShader> SkSVGRadialGradient::onMakeShader(const SkSVGRenderContext& ctx,
    ...
    return center == focal
        ? SkGradientShader::MakeRadial(center, r, colors, pos, count, static_cast<SkShader::TileMode>(tm), 0, &m)
        : SkGradientShader::MakeTwoPointConical(focal, 0, center, r, colors, pos, count,
            static_cast<SkShader::TileMode>(tm), 0, &m);
#else
    return center == focal
