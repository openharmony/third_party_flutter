...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(
    ...
    if (!proxy) {
        ...
        if ((proxy = generator->generateTexture(ctx, this->imageInfo(), fOrigin, willBeMipped))) {
            ...
            if (!willBeMipped || GrMipMapped::kYes == proxy->mipMapped()) {
                // Delete the message in the queue with same key, avoid memory leak.
                for (size_t i = 0; i < fUniqueKeyInvalidatedMessages.size(); i++) {
                    auto message = fUniqueKeyInvalidatedMessages[i];
                    if (key == message->key()) {
                        delete message;
                        fUniqueKeyInvalidatedMessages[i] =
