...
void Canvas::clipPath(const CanvasPath* path, bool doAntiAlias) {
  if (!canvas_)
    return;
  if (!path) {
    FML_LOG(ERROR) << "Canvas.clipPath called with non-genuine Path.";
    return;
  }
...
void Canvas::drawPath(const CanvasPath* path,
  ...
  if (!canvas_)
    return;
  if (!path) {
    FML_LOG(ERROR) << "Canvas.drawPath called with non-genuine Path.";
    return;
  }
...
void Canvas::drawImage(const CanvasImage* image,
  ...
  if (!canvas_)
    return;
  if (!image) {
    FML_LOG(ERROR) << "Canvas.drawImage called with non-genuine Image.";
    return;
  }
...
void Canvas::drawImageRect(const CanvasImage* image,
  ...
  if (!canvas_)
    return;
  if (!image) {
    FML_LOG(ERROR) << "Canvas.drawImageRect called with non-genuine Image.";
    return;
  }
...
void Canvas::drawImageNine(const CanvasImage* image,
  ...
  if (!canvas_)
    return;
  if (!image) {
    FML_LOG(ERROR) << "Canvas.drawImageNine called with non-genuine Image.";
    return;
  }
  SkRect center =
...
void Canvas::drawPicture(Picture* picture) {
  if (!canvas_)
    return;
  if (!picture) {
    FML_LOG(ERROR) << "Canvas.drawPicture called with non-genuine Picture.";
    return;
  }
...
void Canvas::drawPoints(const Paint& paint,
  ...
  canvas_->drawPoints(point_mode,
                      points.size() / 2,  // SkPoints have two floats.
...
void Canvas::drawVertices(const Vertices* vertices,
  ...
  if (!canvas_)
    return;
  if (!vertices) {
    FML_LOG(ERROR) << "Canvas.drawVertices called with non-genuine Vertices.";
    return;
  }
...
void Canvas::drawAtlas(const Paint& paint,
  ...
  if (!canvas_)
    return;
  if (!atlas) {
    FML_LOG(ERROR) << "Canvas.drawAtlas or Canvas.drawRawAtlas called with "
                      "non-genuine Image.";
    return;
  }
  ...
  canvas_->drawAtlas(
      ...
      rects.size() / 4,  // SkRect have four floats.
...
void Canvas::drawShadow(const CanvasPath* path,
  ...
  if (!path) {
    FML_LOG(ERROR) << "Canvas.drawShader called with non-genuine Path.";
    return;
  }
