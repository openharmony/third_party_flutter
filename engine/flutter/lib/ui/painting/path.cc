...
void CanvasPath::addPath(CanvasPath* path, double dx, double dy) {
  if (!path) {
    FML_LOG(ERROR) << "Path.addPath called with non-genuine Path.";
    return;
  }
...
void CanvasPath::addPathWithMatrix(CanvasPath* path,
  ...
  if (!path) {
    FML_LOG(ERROR) << "Path.addPathWithMatrix called with non-genuine Path.";
    return;
  }
...
void CanvasPath::extendWithPath(CanvasPath* path, double dx, double dy) {
  if (!path) {
    FML_LOG(ERROR) << "Path.extendWithPath called with non-genuine Path.";
    return;
  }
