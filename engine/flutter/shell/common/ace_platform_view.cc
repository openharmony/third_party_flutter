...
std::unique_ptr<VsyncWaiter> PlatformView::CreateVSyncWaiter(int32_t platform) {
  FML_DLOG(WARNING)
...
void PlatformView::SetIdleNotificationCallback(const IdleCallback& idleCallback) {
    delegate_.OnSetIdleNotificationCallback(idleCallback);
}
