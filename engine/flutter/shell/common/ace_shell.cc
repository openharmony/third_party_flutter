...
/**
 * Preload font collection for accelerate text layout.
 *
 * txt::FontCollection uses font_collections_cache_ to cache minikin::FontCollection.
 * ParagraphTxt::Layout will calling `getFamilyForChar` for every character. While first encountering
 * characters like Chinese, a fallback will occured. So manually fallback.
 */
void FontCollectionPreload(flutter::Window* window) {
  if (window && window->client()) {
    window->client()->GetFontCollection().GetFontCollection();
  }
}
...
std::unique_ptr<Shell> Shell::CreateShellOnPlatformThread(
  ...
  auto shell =
      std::unique_ptr<Shell>(new Shell(task_runners, settings));
  ...
  auto vsync_waiter = platform_view->CreateVSyncWaiter(static_cast<int32_t>(settings.platform));
  if (!vsync_waiter) {
    return nullptr;
  }
  ...
  fml::TaskRunner::RunNowOrPostTask(
      ...
      fml::MakeCopyable([&ui_latch,                                       //
        ...
        // Window is inited in Engine construction.
        window = flutter::UIDartState::Current()->window();
        ui_latch.Signal();
      }));
  ...
  fml::TaskRunner::RunNowOrPostTask(
        io_task_runner,
        [window]() {
          TRACE_EVENT0("flutter", "FontCollectionPreload");
          // Window mustn't be dangling because Engine must be destroyed after this task. See Shell~Shell().
          FontCollectionPreload(window);
        });
...
Shell::~Shell() {
  ...
  // Wait for IO thread finishing all works before UI thread destroy because we preload FontCollection on IO.
  fml::AutoResetWaitableEvent wait_io_latch;
  fml::TaskRunner::RunNowOrPostTask(
      task_runners_.GetIOTaskRunner(), [&wait_io_latch]() {
        wait_io_latch.Signal();
      });
  wait_io_latch.Wait();
...
void Shell::OnPlatformViewCreated(std::unique_ptr<Surface> surface) {
  ...
  auto io_task = [io_manager = io_manager_->GetWeakPtr(), platform_view,
                  ...
                  should_post_gpu_task, gpu_task_runner = task_runners_.GetGPUTaskRunner(),
                  gpu_task, &latch] {
...
// |PlatformView::Delegate|
void Shell::OnSetIdleNotificationCallback(const Engine::IdleCallback& idleCallback) {
    task_runners_.GetUITaskRunner()->PostTask(
        [engine = engine_->GetWeakPtr(), idleCallback]() {
          if (engine) {
            engine->SetIdleNotificationCallback(idleCallback);
          }
        });
}
