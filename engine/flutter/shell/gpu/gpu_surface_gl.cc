...
// ACE PC preview
#if defined(WINDOWS_PLATFORM) or defined(MAC_PLATFORM) or defined(LINUX_PLATFORM)
    ...
    int32_t pixelSize = info.bytesPerPixel();
    auto pixelBuffer = std::make_unique<char[]>(width * height * pixelSize);
