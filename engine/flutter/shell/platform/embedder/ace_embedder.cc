...
HandleTouchEventCallback g_HandleTouchEventCallback;
...
InferOpenGLPlatformViewCreationCallback(
  ...
  flutter::EmbedderSurfaceGL::GLDispatchTable gl_dispatch_table = {
      ...
      gl_send_surface,                     // gl_send_surface_callback
...
FlutterEngineResult FlutterEngineRun(const FlutterRendererConfig* config,
                                     ...
                                     FLUTTER_API_SYMBOL(FlutterEngine) *
                                         engine_out) {
  auto result =
      FlutterEngineInitialize(config, args, user_data, engine_out);
...
FlutterEngineResult FlutterEngineSetIdleNotificationCallback(
    ...
    if (engine == nullptr || idle_notification_callback == nullptr) {
        return LOG_EMBEDDER_ERROR(kInvalidArguments);
    }

    return reinterpret_cast<flutter::EmbedderEngine*>(engine)->SetIdleNotificationCallback(
...
#ifdef USE_GLFW_WINDOW
  if (g_HandleTouchEventCallback && g_HandleTouchEventCallback(packet)) {
    return kSuccess;
  }
...
void FlutterEngineRegisterHandleTouchEventCallback(HandleTouchEventCallback&& callback) {
  g_HandleTouchEventCallback = std::move(callback);
}
#endif
