...
bool AndroidContextGL::CreateWindowSurface(
  ...
  // If surface not empty, create a new one will lead mem-leak in mali driver.
  // So should teardown the old surface before create a new one.
  TeardownSurface(display, surface_);
...
bool AndroidContextGL::MakeCurrent() {
  // If same context just return, avoid unused egl call.
